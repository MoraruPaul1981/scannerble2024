package com.sous.server.businesslayer.bl_startingservices;
...
import com.sous.server.businesslayer.Services.AdvertiseCallbackService;
import com.sous.server.businesslayer.Services.ServiceServerGattServer;
import com.sous.server.businesslayer.bl_powermanager.GetPowerMaanger;
...
public class BusinesslogicStartingService {
    ...
    public void startingServiceGattServer( ) {
        try {
            ...
            // TODO: 15.08.2024
            ServiceGattServerScan=  new GetPowerMaanger(context,version). startPowerManager(ServiceGattServerScan);
            ...
            Integer ЛокальнаяВерсияПОСравнение = Integer.parseInt(ТекущаяВерсияПрограммы.toString());
            valuesЗаписываемОшибки.put("whose_error", ЛокальнаяВерсияПОСравнение);
            new com.sous.server.businesslayer.Errors.SubClassErrors(context).МетодЗаписиОшибок(valuesЗаписываемОшибки);
        }
