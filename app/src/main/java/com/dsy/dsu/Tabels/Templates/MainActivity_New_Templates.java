...
public class MainActivity_New_Templates extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    private  FloatingActionButton floatingactionbuttontemplates;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ///////// todo Круглая Кнопка
            floatingactionbuttontemplates = findViewById(R.id.floatingactionbuttontemplates);//////КНОПКА СОЗДАНИЕ НОВГО ТАБЕЛЯ ИЗ ИСТОРИИ ВТОРОЙ ШАГ СОЗДАНИЯ ТАБЕЛЯ СНАЧАЛА ИСТРОИЯ ПОТОМ НА БАЗЕ ЕГО СОЗЗДАНИЕ
    ...
    private void МетодДляКруглойКнопки() {
        floatingactionbuttontemplates.setOnClickListener(new View.OnClickListener() {
            @Override
            ...
            if (Курсор_КоторыйЗагружаетГотовыеШаблоны.getCount() > 0) {/////ЕСЛИ ЕСТЬХОТЯБЫ ОДИН ТАБЕЛЬ
               ...
               /* Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                ...
                do {
                    ...
                    ШАблонвВидеКнопок = new Button(getApplicationContext());////СОЗДАЕМ НОВЫЕ КНОПКИ НА АКТИВТИ
                    //////
    ...
    private ContentValues МетодЗаполениеяДаннымиСотрудникаДляШаблонаЕслиОниЕсть(ContentValues АдаптерДляВставкиИзГотоваШаблонаВТаблицуТабель,
        ...
        try{
            ...
            // TODO: 27.08.2021  ПОЛУЧЕНИЕ ДАННЫХ ОТ КЛАССА GRUD-ОПЕРАЦИИ
                ...
                @SuppressLint("Range") Long ФИОИзАТблицы=  Курсор_СамиДАнные.getLong(Курсор_СамиДАнные.getColumnIndex("fio_uuid"));
