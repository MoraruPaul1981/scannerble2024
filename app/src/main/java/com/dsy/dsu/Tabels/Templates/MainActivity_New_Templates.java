package com.dsy.dsu.Tabels.Templates;
...
@AndroidEntryPoint
public class MainActivity_New_Templates extends AppCompatActivity {
    ...
    private  FloatingActionButton floatingactionbuttoncustomers;
    ...
    private   TextView textviewcountcustomer;
...
///////TODO
            ...
            СпинерДата.setEnabled(false);

            СпинерЦФО = (Spinner) findViewById(R.id.СпинерТабельДепратамент);
            СпинерЦФО.startAnimation(animation);
            ...
            /////////Круглая Кнопка
            floatingactionbuttoncustomers = findViewById(R.id.floatingactionbuttoncustomers);//////КНОПКА СОЗДАНИЕ НОВГО ТАБЕЛЯ ИЗ ИСТОРИИ ВТОРОЙ ШАГ
            ...
            listActivityListPeoples.startAnimation(animationv3);
            // ОбщееКоличествоВТабелеСотудников
            textView3СловоТабель = (TextView) findViewById(R.id.textView3СловоТабель);
            textviewcountcustomer = (TextView) findViewById(R.id.textviewcountcustomer);
            ...
            getfloatingactionbuttoncustomers();
    ...
    private void getfloatingactionbuttoncustomers( ) {
        try {
            floatingactionbuttoncustomers.setOnClickListener(new View.OnClickListener() {
                @Override
...
/////TODO КНОПКА КРУГЛАЯ НА АКТИВТИ В ТАБЕЛЕ ПРЕДНАЗВАНЧЕН ДЛЯ СОЗДАНИЕ НОВОГО СОТРУДНИКА ИЛИ ДОБАВЛЕНИЕ УЖЕ ДЕЙСТВУЕЩЕГО СОТРУДКА В ТАБЕЛЬ
                    методMessageДобавлениеСотрудников( );
                    Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
                            " metod " + Thread.currentThread().getStackTrace()[2].getMethodName() + "\n" +
                            " line " + Thread.currentThread().getStackTrace()[2].getLineNumber() + "\n"
...
// TODO: 10.04.2023 получаем данные курсор\
    ...
    private void getRowPeople() {
        try{
            ПолученыеКоличествоСторочек=Курсор_ВсеСотрудникиТабеля.getCount();
            Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
                    " metod " + Thread.currentThread().getStackTrace()[2].getMethodName() + "\n" +
                    " line " + Thread.currentThread().getStackTrace()[2].getLineNumber() + "\n"
                    + " ПолученыеКоличествоСторочек "+ПолученыеКоличествоСторочек );
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                    " Линия  :" + Thread.currentThread().getStackTrace()[2].getLineNumber());
...
//                        }
    ...
    private void МетолСпинерДаты() {
        try {
            if (ИмесяцвИГодСразу!=null) {
                ArrayList<String>  МассивДляСпинераДаты=new ArrayList<>();
                МассивДляСпинераДаты.add(ИмесяцвИГодСразу);
    ...
    void setTextviewCountCustomer() {
        try {
            if (Курсор_ВсеСотрудникиТабеля.getCount()>0) {
                Integer gettextviewcountcustomer=     Курсор_ВсеСотрудникиТабеля.getCount();
                textviewcountcustomer.setText("("+gettextviewcountcustomer.toString()+")");
            }else {
                String dotncount="("+"0"+")";
                textviewcountcustomer.setText(dotncount);
            }
            // TODO: 24.02.2025
            textviewcountcustomer.requestLayout();
            Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
                    " metod " + Thread.currentThread().getStackTrace()[2].getMethodName() + "\n" +
                    " line " + Thread.currentThread().getStackTrace()[2].getLineNumber() + "\n"
                    + " FullNameCFO "+FullNameCFO+ " MainParentUUID " +MainParentUUID
    ...
    private void методСпинерЦФО( ) {
        try{
            if (FullNameCFO!=null) {
                ...
                ArrayList<String> МассивДляСпинераЦФО=new ArrayList<>();
                ...
                СпинерЦФО.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                    ...
                    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                        try {
                            if (СпинерЦФО.getCount()>0  && ((TextView) parent.getChildAt(0))!=null  ) {
    ...
    ///////
    public void методMessageДобавлениеСотрудников( ) {//  ////MESSAGEBOX ДЛЯ ИНТЕРНЕТА  ПОДКЛЮЧЕНИЕ не успешное нет связи с итрнтнето и/или файлом json
        try{
            int ЦветЗначек;
            ...
            ////
            final AlertDialog getNewCustomerDialogBox = new MaterialAlertDialogBuilder(this)
                    .setTitle("Добавить")
            ...
            КнопкаВыбратьСотрудника=getNewCustomerDialogBox.findViewById(R.id.textView6YcheGotobCustomers);
            ...
            КнопкаВыбратьСотрудника.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    try{
                        ...
                        message.getTarget().postDelayed(()->{
                            getNewCustomerDialogBox.dismiss();
                            Log.d(this.getClass().getName(), " " );
                            Intent ИнтентВыбратьСотрудника = new Intent();
                            ИнтентВыбратьСотрудника.setClass(getApplicationContext(), MainActivity_Find_Peoples.class);
...
/////TODO КНОПКА СОЗДАНИЯ НОВОГО СОТРУДКНИКА
            КнопкаНовыйСотрудник=getNewCustomerDialogBox.findViewById(R.id.textView1NewCustomers);
                ...
                public void onClick(View v) {
                    ...
                    try{
                        ...
                        message.getTarget().postDelayed(()->{
                            ...
                            ИнтентНовыйСотрудник.setClass(getApplicationContext(), MainActivity_New_People.class);
...
///////////////////////////TODO ЧЕТВЕРТАЯ КНОПКА
            КнопкаИзШаблона=getNewCustomerDialogBox.findViewById(R.id.textViewFromISCHABLONA);
...
///////////////////////////TODO ПЯТАЯ КНОПКА
            СкопироватьТабель=getNewCustomerDialogBox.findViewById(R.id.textView3SapolnitFrrombackmezaz);
            СкопироватьТабель.setPadding(0,0,60,0);
            СкопироватьТабель.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    try{
                        // TODO: 24.02.2025
                        СкопироватьТабель.toggle();
                        progressDialog= new ProgressDialog(activity);
                        progressDialog.setIndeterminate(true);
                        progressDialog.setTitle("Табель");
                        ...
                        progressDialog.setMessage("Копирование табеля...");
                        ...
                        try {
                            ...
                            Log.d(this.getClass().getName(),"\n" + " class " + Thread.currentThread().getStackTrace()[2].getClassName() + "\n" +
                                    ...
                                    " line " + Thread.currentThread().getStackTrace()[2].getLineNumber() + "\n");
                            getNewCustomerDialogBox.dismiss();
                            getNewCustomerDialogBox.cancel();
                        } catch (Exception e) {
                            e.printStackTrace();
                            Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() +
                                    " Линия  :" + Thread.currentThread().getStackTrace()[2].getLineNumber());
            ...
            КнопкаВыход=getNewCustomerDialogBox.findViewById(R.id.textView7Exit);
            ...
            getNewCustomerDialogBox.show();
            Rect displayRectangle = new Rect();
            Window window =  getWindow();
            window.getDecorView().getWindowVisibleDisplayFrame(displayRectangle);
            getNewCustomerDialogBox.getWindow().setLayout((int) (displayRectangle.width() *
                    1f), (int) (displayRectangle.height()*0.7f));


            // TODO: 24.02.2025
            // getNewCustomerDialogBox.getWindow().setLayout(600, 400); // It didn't work.
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(this.getClass().getName(), "Ошибка " + e + " Метод :" + Thread.currentThread().getStackTrace()[2].getMethodName() + " Линия  :"
                    + Thread.currentThread().getStackTrace()[2].getLineNumber());
            ...
            try{
                IntStream.iterate(1, i -> i + 1).limit(31 ).parallel().forEachOrdered(new IntConsumer() {
                    ...
                    public void accept(int valueЧасы) {
                        ...
                        @SuppressLint("Range") String ЧасыОтработанные=   cursorGetChasy.getString(cursorGetChasy.getColumnIndex(Дни));
